...
const Divider = ({ dim = false }) => (
  <div style={{ height:1, background: dim ? "#1a1a1a" : C.border, width:"100%" }} />
)
...
const ScanLine = () => (
  <div style={{
    position:"fixed", left:0, right:0, height:80, pointerEvents:"none", zIndex:9999,
    background:"linear-gradient(180deg,transparent,rgba(161,217,198,0.018),transparent)",
...
// ─── Main ─────────────────────────────────────────────────────────
export default function YungDrip() {
  const [filter, setFilter]           = useState("ALL")
  const [cart, setCart]               = useState([])
  const [cartOpen, setCartOpen]       = useState(false)
  const [aiOpen, setAiOpen]           = useState(false)
  const [aiProduct, setAiProduct]     = useState(null)
  const [aiStep, setAiStep]           = useState(0) // 0=form 1=loading 2=results
  ...
  useEffect(() => {
    ...
    const msgs = [
      "READING PROFILE...",
      "CROSS-REFERENCING 12K+ LOOKS...",
      "MAPPING TONE PALETTE...",
      "BUILDING YOUR FIT...",
    ]
    let i=0, charI=0
    setAiMsg("")
    const loop = () => {
      ...
      if(charI <= msgs[i].length) {
        setAiMsg(msgs[i].slice(0,charI))
        charI++
        setTimeout(loop, 38)
      } else {
        setTimeout(()=>{ i++; charI=0; loop() },500)
      }
    }
    loop()
  ...
  const openAI = (p) => {
    setAiProduct(p||PRODUCTS[0])
    setAiStep(0)
    setAiForm({gender:"",skin:"",body:"",style:""})
    setAiRecs([])
    setLoadPct(0)
    setAiOpen(true)
  }
  ...
  return (<>
    <style>{`
      ...
      *{box-sizing:border-box;margin:0;padding:0;}
      ...
      .hover-lift{transition:transform 0.4s cubic-bezier(0.23,1,0.32,1),box-shadow 0.4s;}
      .hover-lift:hover{transform:translateY(-6px);box-shadow:0 20px 60px rgba(0,0,0,0.7);}
      ...
      .nav-item:hover,.nav-item.active{color:${C.fg};}
      ...
      .cat-btn.off:hover{color:${C.fg};border-color:#333;}

      .size-chip{
        font-family:'Courier New',monospace;font-size:10px;
        border:1px solid #222;padding:5px 9px;
      ...
      .ai-opt{
        font-family:'Dela Gothic One',sans-serif;font-size:11px;letter-spacing:0.12em;
        border:1px solid #1f1f1f;padding:12px 14px;
        cursor:pointer;transition:all 0.18s;color:#555;
        background:transparent;text-align:left;
        display:flex;align-items:center;gap:10px;
      }
      .ai-opt:hover,.ai-opt.on{border-color:${C.accent};color:${C.fg};background:rgba(161,217,198,0.04);}
      .ai-opt.on::before{content:'✓';color:${C.accent};font-size:9px;}

      .panel-enter{animation:slideRight 0.38s cubic-bezier(0.23,1,0.32,1);}
    `}</style>
    ...
    <nav style={{
      ...
      <div style={{ background:C.secondary, overflow:"hidden", height:28, display:"flex", alignItems:"center" }}>
        <div style={{ display:"flex", gap:0, animation:"marquee 22s linear infinite", whiteSpace:"nowrap" }}>
          {Array(2).fill(["FREE SHIPPING OVER ₹4999","·","NEW DROP: SPECTRE PUFFER","·","USE CODE DRIP20 FOR 20% OFF","·","DROP 002 COMING FRIDAY","·"]).flat().map((w,i)=>(
      ...
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"0 40px", height:62 }}>
        ...
        <div style={{ display:"flex", gap:36 }}>
          {["SHOP","DROPS","ABOUT"].map(l=>(
            <button key={l} className="nav-item">{l}</button>
          ))}
        </div>
        <div style={{ display:"flex", alignItems:"center", gap:20 }}>
          ...
          <button
            ...
            style={{ padding:"9px 18px", display:"flex", alignItems:"center", gap:7, fontSize:10 }}
            onClick={()=>openAI(null)}>
            <span style={{ fontSize:12 }}>◈</span> AI STYLIST
          </button>
          {/* Cart */}
          <button onClick={()=>setCartOpen(true)} style={{ position:"relative", background:"none", border:"none", cursor:"pointer", color:C.muted, transition:"color 0.2s", display:"flex" }}
    ...
    <section style={{ minHeight:"100vh", display:"flex", flexDirection:"column", justifyContent:"flex-end", position:"relative", overflow:"hidden" }}>
      ...
      <img src="https://images.unsplash.com/photo-1509631179647-0177331693ae?w=1600&h=900&fit=crop&crop=top"
        style={{ position:"absolute",inset:0,width:"100%",height:"100%",objectFit:"cover",filter:"brightness(0.38) contrast(1.2)",zIndex:0 }} alt="" />
      {/* Gradient overlay */}
      ...
      <div style={{ position:"relative",zIndex:3,padding:"0 80px 80px",opacity:heroIn?1:0,transform:heroIn?"translateY(0)":"translateY(30px)",transition:"all 1s cubic-bezier(0.23,1,0.32,1)" }}>
    ...
    <div style={{ borderTop:"1px solid #111",borderBottom:"1px solid #111",padding:"13px 0",overflow:"hidden",background:"#050505" }}>
      <div style={{ display:"flex",gap:0,animation:"marquee 18s linear infinite",whiteSpace:"nowrap" }}>
        {Array(2).fill(["YUNGDRIP","—","STREETWEAR","—","FOR THOSE AT THE FRINGE","—","DROP 001","—","AI-STYLED","—","CULTURE","—","VOID","—"]).flat().map((w,i)=>(
          <span key={i} className="dela" style={{ fontSize:11,letterSpacing:"0.3em",color:i%2===0?"#222":"#111",marginRight:48,textTransform:"uppercase" }}>{w}</span>
        ))}
      </div>
    </div>
    ...
    <section id="shop" style={{ padding:"100px 40px 120px",maxWidth:1360,margin:"0 auto" }}>
      ...
      <div style={{ display:"flex",alignItems:"flex-end",justifyContent:"space-between",marginBottom:56,flexWrap:"wrap",gap:20 }}>
        ...
        <div style={{ display:"flex",gap:8,flexWrap:"wrap" }}>
          {cats.map(c=>(
            <button key={c} className={`cat-btn ${filter===c?"on":"off"}`}
            ...
            <div style={{ padding:"18px 18px 20px" }}>
              ...
              <div style={{ display:"flex",gap:4,flexWrap:"wrap",marginBottom:16 }}>
                {p.sizes.slice(0,5).map(s=>(
                  <button key={s}
                    className={`size-chip ${selectedSizes[p.id]===s?"on":""}`}
              ...
              <div style={{ display:"flex",gap:10 }}>
                <button className="btn-accent"
                  style={{ flex:1,padding:"11px 14px",fontSize:10,
                    background:addedId===p.id?"#fff":"rgba(161,217,198,0.85)",
                    transition:"all 0.3s" }}
                  onClick={()=>addToCart(p)}>
                  {addedId===p.id?"✓ ADDED":"ADD TO CART"}
                </button>
                <button className="btn-outline" style={{ padding:"11px 16px",fontSize:10 }}
                  onClick={()=>openAI(p)}>◈</button>
              </div>
            </div>
          </div>
        ))}
    ...
    <section style={{ background:C.secondary,padding:"80px 40px",position:"relative",overflow:"hidden" }}>
      ...
      <div className="dela" style={{ position:"absolute",top:"50%",left:"50%",transform:"translate(-50%,-50%)",fontSize:"clamp(80px,16vw,200px)",color:"rgba(161,217,198,0.04)",whiteSpace:"nowrap",pointerEvents:"none",letterSpacing:"-0.02em" }}>AI STYLIST</div>
      ...
      <div style={{ maxWidth:900,margin:"0 auto",textAlign:"center",position:"relative",zIndex:1 }}>
        ...
        <p className="bask" style={{ fontSize:16,color:"#9abfb5",lineHeight:1.8,maxWidth:560,margin:"0 auto 36px",fontStyle:"italic" }}>
          ...
          to curate a fit that's distinctly yours — not the algorithm's.
        </p>
    ...
    <footer style={{ background:"#000",borderTop:"1px solid #111",padding:"60px 40px 36px" }}>
      <div style={{ maxWidth:1360,margin:"0 auto" }}>
            ...
            <div key={col.t}>
              ...
              {col.ls.map(l=>(
                <div key={l} className="bask" style={{ fontSize:13,color:C.muted,cursor:"pointer",marginBottom:10,transition:"color 0.15s",fontStyle:"italic" }}
                  onMouseEnter={e=>e.target.style.color=C.fg} onMouseLeave={e=>e.target.style.color=C.muted}>{l}</div>
              ))}
            </div>
          ))}
        </div>
        ...
        <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",paddingTop:24,flexWrap:"wrap",gap:12 }}>
          ...
          <span className="mono" style={{ fontSize:9,color:C.secondary,letterSpacing:"0.2em" }}>AI: LIVE · DROP: ACTIVE · SYSTEM: ONLINE</span>
        </div>
      </div>
    </footer>
      ...
      <div style={{ position:"fixed",inset:0,zIndex:600 }}>
        ...
        <div className="panel-enter" style={{
          ...
          <div style={{ padding:"22px 24px",borderBottom:"1px solid #111",display:"flex",justifyContent:"space-between",alignItems:"center" }}>
            ...
            <button onClick={()=>setCartOpen(false)} className="btn-ghost-sm" style={{ fontSize:18 }}>✕</button>
          </div>
          <div style={{ flex:1,overflow:"auto",padding:"18px 24px" }}>
            {cart.length===0?(
              <div style={{ textAlign:"center",padding:"70px 0",color:C.muted }}>
                <div className="dela" style={{ fontSize:14,letterSpacing:"0.15em",marginBottom:8 }}>EMPTY BAG</div>
                <div className="bask" style={{ fontSize:13,fontStyle:"italic",color:"#333" }}>Your cart is empty, go fill it up.</div>
              </div>
            ):cart.map(item=>(
              <div key={`${item.id}-${item.size}`} style={{ display:"flex",gap:14,padding:"16px 0",borderBottom:"1px solid #0f0f0f" }}>
                <img src={item.img} alt={item.name} style={{ width:74,height:86,objectFit:"cover",filter:"brightness(0.8)" }}/>
                <div style={{ flex:1 }}>
                  <div className="dela" style={{ fontSize:13,letterSpacing:"0.04em",marginBottom:4 }}>{item.name}</div>
                  <div className="mono" style={{ fontSize:9,color:C.muted,letterSpacing:"0.15em",marginBottom:8 }}>
                    {item.size} · QTY {item.qty}
                  </div>
                  <PriceTag price={item.price*item.qty}/>
                </div>
                <button className="btn-ghost-sm" style={{ alignSelf:"flex-start",fontSize:16 }}
          ...
          {cart.length>0&&(
            <div style={{ padding:"18px 24px",borderTop:"1px solid #111" }}>
              ...
              <button className="btn-accent" style={{ width:"100%",padding:"15px",fontSize:12 }}>
                CHECKOUT →
              </button>
            </div>
          )}
    ...
    {/* ─────────────────────────────────────────────────────────────
        AI STYLIST PANEL — FULL FEATURED
    ───────────────────────────────────────────────────────────── */}
    {aiOpen && (
          ...
          <div style={{ padding:"22px 28px 18px",borderBottom:"1px solid #111",flexShrink:0 }}>
            <div style={{ display:"flex",justifyContent:"space-between",alignItems:"flex-start" }}>
              ...
              <button onClick={()=>setAiOpen(false)}
                ...
                onMouseEnter={e=>{e.currentTarget.style.borderColor="#333";e.currentTarget.style.color=C.fg}}
          ...
          <div style={{ flex:1,overflow:"auto",padding:"24px 28px" }}>
            ...
            {aiStep===0 && (
              <div style={{ animation:"fadeIn 0.35s ease" }}>
                ...
                <div style={{ marginBottom:28 }}>
                  <div className="mono" style={{ fontSize:9,color:C.accent,letterSpacing:"0.35em",marginBottom:12 }}>
                    01 — IDENTITY
                  </div>
                  <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6 }}>
                    {[{v:"male",l:"MALE"},{v:"female",l:"FEMALE"},{v:"fluid",l:"GENDER FLUID"}].map(g=>(
                      <button key={g.v} className={`ai-opt ${aiForm.gender===g.v?"on":""}`}
                        onClick={()=>setAiForm(f=>({...f,gender:g.v}))}
                ...
                <div style={{ marginBottom:28 }}>
                  ...
                  <div style={{ display:"flex",gap:10,alignItems:"center",flexWrap:"wrap" }}>
                    {[
                      {v:"fair",c:"#f5d5b0",label:"Fair"},
                      {v:"light",c:"#e8b98a",label:"Light"},
                      {v:"medium",c:"#c8855a",label:"Medium"},
                      {v:"tan",c:"#a0622b",label:"Tan"},
                      {v:"brown",c:"#7a4020",label:"Brown"},
                      {v:"deep",c:"#3d1e0e",label:"Deep"},
                    ].map(s=>(
                      <div key={s.v} style={{ display:"flex",flexDirection:"column",alignItems:"center",gap:5,cursor:"pointer" }}
                        onClick={()=>setAiForm(f=>({...f,skin:s.v}))}>
                        <div style={{
                          width:34,height:34,borderRadius:"50%",background:s.c,
                          border:`2px solid ${aiForm.skin===s.v?C.accent:"transparent"}`,
                          boxShadow:aiForm.skin===s.v?`0 0 10px ${C.accent}55`:"none",
                          transition:"all 0.18s",
                        }}/>
                        <span className="mono" style={{ fontSize:8,color:aiForm.skin===s.v?C.accent:"#333",letterSpacing:"0.1em" }}>{s.label}</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div style={{ marginBottom:28 }}>
                  ...
                  <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6 }}>
                    {[
                      {v:"slim",l:"SLIM",d:"Lean frame"},
                      {v:"athletic",l:"ATHLETIC",d:"Muscular"},
                      {v:"broad",l:"BROAD",d:"Fuller build"},
                    ].map(b=>(
                      <button key={b.v} className={`ai-opt ${aiForm.body===b.v?"on":""}`}
                        ...
                        <span className="bask" style={{ fontSize:10,color:"#444",fontStyle:"italic",marginLeft:aiForm.body===b.v?16:0,transition:"margin 0.2s" }}>{b.d}</span>
                      </button>
                    ))}
                  </div>
                </div>
                ...
                <button className="btn-accent"
                  style={{
                    ...
                    opacity:aiForm.gender&&aiForm.skin&&aiForm.body&&aiForm.style?1:0.25,
                ...
                <p className="bask" style={{ fontSize:11,color:"#2a2a2a",textAlign:"center",marginTop:14,fontStyle:"italic" }}>
                  Your profile is private and never stored.
                </p>
              </div>
            )}
              ...
              <div style={{ animation:"fadeIn 0.3s ease",textAlign:"center",padding:"40px 0" }}>
                ...
                <div style={{ position:"relative",display:"inline-block",marginBottom:36 }}>
                  <svg width="100" height="100" viewBox="0 0 100 100"
                    ...
                    <circle cx="50" cy="50" r="44" fill="none" stroke={C.accent} strokeWidth="1"
                ...
                <div className="mono" style={{ fontSize:11,color:C.accent,letterSpacing:"0.12em",minHeight:18,marginBottom:8 }}>
                  {aiMsg}<span style={{ animation:"pulse 0.8s step-end infinite",display:"inline" }}>_</span>
                </div>
                {/* Progress bar */}
                ...
                <div className="mono" style={{ fontSize:8,color:"#222",letterSpacing:"0.2em",marginTop:20 }}>
                  CROSS-REFERENCING 14,000+ STYLE PROFILES
                </div>
              </div>
            )}
            ...
            {aiStep===2 && (
              <div style={{ animation:"fadeUp 0.4s cubic-bezier(0.23,1,0.32,1)" }}>
                ...
                <p className="bask" style={{ fontSize:12,color:C.muted,fontStyle:"italic",marginBottom:24 }}>
                  Curated specifically for your profile. Tap to add to bag.
                </p>
                ...
                {aiRecs.map((rec,i)=>(
                  <div key={rec.id} style={{
                    ...
                    opacity:0,
                    animation:`fadeUp 0.45s cubic-bezier(0.23,1,0.32,1) ${i*0.1}s forwards`
                  }}>
                ...
                <div style={{ marginTop:24,padding:"16px",background:C.secondary,border:"1px solid #283e36" }}>
                  ...
                  <p className="bask" style={{ fontSize:11,color:"#7aab9a",fontStyle:"italic",marginBottom:12 }}>
                    Get the complete look — all 3 picks added at once.
                  </p>
